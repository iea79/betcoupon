(function(window, undefined) {
    ...
    function baron(params) {
        ...
        // this - global context or jQuery instance
        if (jQueryMode) {
    ...
    function arrayEach(obj, iterator) {
        var i = 0;
        if (obj.length === undefined || obj === window)
            obj = [obj];
        while (obj[i]) {
    ...
    function manageEvents(item, eventManager, mode) {
        ...
        item._eventHandlers = item._eventHandlers || [{
            ...
            handler: function(e) {
                e.preventDefault();
                // Text selection disabling in Opera
                item.selection();
    ...
    function init(params) {
        ...
        out.scrollEdge = 0;
        if (params.rtl) {
            out.scrollEdge = out.clipper[out.origin.scrollEdge];
            // initial position
        }
        return out;
    }
    ...
    item.prototype = {
        ...
        _debounce: function(func, wait) {
            var self = this, timeout, // args, // right now there is no need for arguments
        ...
        constructor: function(params) {
            ...
            this.rpos = function(r) {
                ...
                var free = this.scroller[this.origin.scrollSize] - this.scroller[this.origin.client], x;
                if (r) {
                    x = this.pos(r * free);
                } else {
                    x = this.pos();
            ...
            this.resize = function() {
                ...
                function upd() {
                    ...
                    if (offset) {
                        ...
                        var fixScroller = self.impact ? (self.impact == 'scroller') : (self.direction == 'v');
                        if (fixScroller) {
                            // scroller
    ...
    baron.noConflict = function() {
        window.baron = _baron;
        // Restoring original value of "baron" global var
        return baron;
    }
    ;
    baron.version = '1.2.1';
...
(function(window, undefined) {
    var fix = function(userParams) {
        ...
        scroller = this.scroller, eventManager = this.event, $ = this.$, self = this;
        ...
        function init(_params) {
            ...
            if (elements) {
                ...
                for (var i = 0; i < elements.length; i++) {
                    ...
                    // Not zero because of negative margins
            ...
            var event = {
                element: elements,
                handler: function() {
                    var parent = $(this)[0].parentNode, top = parent.offsetTop, num;
...
(function(window, undefined) {
    var controls = function(params) {
        var forward, backward, track, screen, self = this, // AAAAAA!!!!!11
        event;
...
(function(window, undefined) {
    var pull = function(params) {
        var block = this.$(params.block), size = params.size || this.origin.size, limit = params.limit || 80, onExpand = params.onExpand, elements = params.elements || [], inProgress = params.inProgress || '', self = this, _insistence = 0, _zeroXCount = 0, _interval, _timer, _x = 0, _onExpandCalled, _waiting = params.waiting || 500, _on;
        function getSize() {
        ...
        function update() {
            var pos = {}, height = getSize(), scrollHeight = getContentSize(), dx, op4, scrollInProgress = _insistence == 1;
            op4 = 0;
            // Возвращающая сила
            if (_insistence > 0) {
                op4 = 40;
